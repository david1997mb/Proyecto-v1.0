...
onMounted( async ()=>{
    const info = await axios.get('/api/eventsbyclients');
    info.data.map((row)=>(
        clients.value.push(row.name),
        events.value.push(row.count),
        types.value.push(row.type),
  ...
  labels: types.value,
  datasets: [
    { label: 'Eventos', data: events.value, backgroundColor: colors },
  ]
};
    loaded.value = true;
})
...
<template>
    <div class="container-fluid">
        <div class="col-md-8 offset-md-2 text-center">
            <h3 class="h3">Eventos por Clientes</h3>
